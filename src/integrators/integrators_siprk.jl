...
function TableauSIPRK(name::Symbol, qorder_drift::Int, qa_drift::Matrix{T},
        qb_drift::Vector{T}, qc_drift::Vector{T}, qorder_diff::Int,
        qa_diff::Matrix{T}, qb_diff::Vector{T}, qc_diff::Vector{T},
        porder_drift::Int, pa_drift::Matrix{T}, pb_drift::Vector{T}, pc_drift::Vector{T}, porder_diff::Int,
        pa_diff::Matrix{T}, pb_diff::Vector{T}, pc_diff::Vector{T}) where {T}
    TableauSIPRK{T}(
        name, length(qc_drift), Tableau(name, qorder_drift, qa_drift, qb_drift, qc_drift),
...
mutable struct ParametersSIPRK{DT, TT, D, M, S, ET <: NamedTuple} <: Parameters{DT, TT}
    ...
    function ParametersSIPRK{DT, D, M}(
            equ::ET, tab::TableauSIPRK{TT}, Δt::TT, ΔW::Vector{DT},
            ΔZ::Vector{DT}, A::DT) where {DT, TT, D, M, ET <: NamedTuple}
        @assert M == length(ΔW) == length(ΔZ)
        new{DT, TT, D, M, tab.s, ET}(
            equ, tab, Δt, ΔW, ΔZ, A, zero(TT), zeros(DT, D), zeros(DT, D))
    end
end
...
struct IntegratorCacheSIPRK{DT, D, M, S} <: PSDEIntegratorCache{DT, D, M}
    ...
    function IntegratorCacheSIPRK{DT, D, M, S}() where {DT, D, M, S}
        ...
        new(Q, P, V, F, B, G, Y, Z, v, f, b, g, y, z, V1, V2, F1,
            F2, B1, B2, G1, G2, Δw, ΔQ, Y1, Y2, ΔP, Z1, Z2, Δy, Δz)
    end
end

function Integrators.IntegratorCache{ST}(
        params::ParametersSIPRK{DT, TT, D, M, S}; kwargs...) where {ST, DT, TT, D, M, S}
    IntegratorCacheSIPRK{ST, D, M, S}(; kwargs...)
end

@inline Integrators.CacheType(ST,
    params::ParametersSIPRK{DT, TT, D, M, S}) where {
    DT, TT, D, M, S} = IntegratorCacheSIPRK{ST, D, M, S}
...
struct IntegratorSIPRK{DT, TT, D, M, S,
    ...
    function IntegratorSIPRK(params::ParametersSIPRK{DT, TT, D, M, S},
            solver::ST, caches) where {DT, TT, D, M, S, ST}
        new{DT, TT, D, M, S, typeof(params), ST}(params, solver, caches)
    end

    function IntegratorSIPRK{DT, D, M}(equations::NamedTuple, tableau::TableauSIPRK{TT},
            Δt::TT; K::Int = 0) where {DT, TT, D, M}
        ...
        # create params
        params = ParametersSIPRK{DT, D, M}(
            equations, tableau, Δt, zeros(DT, M), zeros(DT, M), A)
    ...
    function IntegratorSIPRK(equation::PSDE{DT, TT}, tableau::TableauSIPRK{TT},
            Δt::TT; kwargs...) where {DT, TT}
...
function initial_guess!(int::IntegratorSIPRK{DT, TT}, sol::AtomicSolutionPSDE{DT, TT},
    ...
    for i in eachstage(int)
        ...
        @. cache.Q[i] = int.params.q + 2.0 / 3.0 * Δt_local * cache.V1 +
                        2.0 / 3.0 * cache.ΔQ
        @. cache.P[i] = int.params.p + 2.0 / 3.0 * Δt_local * cache.F1 +
                        2.0 / 3.0 * cache.ΔP

        t2 = int.params.t + 2.0 / 3.0 * Δt_local
        ...
        for j in eachdim(int)
            int.solver.x[(i - 1) * ndims(int) + j] = Δt_local*(1.0 / 4.0 * cache.V1[j] +
                                                               3.0 / 4.0 * cache.V2[j]) +
                                                     1.0 / 4.0 * cache.Y1[j] +
                                                     3.0 / 4.0 * cache.Y2[j]
        end
        ...
        if tableau(int).qdrift.c==tableau(int).pdrift.c
            for j in 1:ndims(int)
                int.solver.x[(tableau(int).s + i - 1) * ndims(int) + j] = Δt_local*(1.0 /
                                                                                    4.0 *
                                                                                    cache.F1[j] +
                                                                                    3.0 /
                                                                                    4.0 *
...
function compute_stages!(x::Vector{ST}, Q::Vector{Vector{ST}}, P::Vector{Vector{ST}},
    ...
    for i in 1:S
        @assert D == length(Q[i]) == length(V[i]) == size(B[i], 1) == length(P[i]) ==
                length(F[i]) == size(G[i], 1)
...
function Integrators.function_stages!(x::Vector{ST}, b::Vector{ST},
    ...
    compute_stages!(
        x, cache.Q, cache.P, cache.V, cache.F, cache.B, cache.G, cache.Y, cache.Z, params)
...
function update_solution!(
        sol::AtomicSolutionPSDE{T}, tab::TableauSIPRK{T}, Δt::T, ΔW::Vector{T},
...
function Integrators.integrate_step!(
        int::IntegratorSIPRK{DT, TT}, sol::AtomicSolutionPSDE{DT, TT},
        cache::IntegratorCacheSIPRK{DT} = int.caches[DT]) where {DT, TT}
    ...
    compute_stages!(int.solver.x, cache.Q, cache.P, cache.V, cache.F,
        cache.B, cache.G, cache.Y, cache.Z, int.params)
