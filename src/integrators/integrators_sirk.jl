...
function TableauSIRK(name::Symbol, order_drift::Int, a_drift::Matrix{T},
        b_drift::Vector{T}, c_drift::Vector{T}, order_diff::Int,
        a_diff::Matrix{T}, b_diff::Vector{T}, c_diff::Vector{T}) where {T}
    TableauSIRK{T}(
        name, length(c_drift), Tableau(name, order_drift, a_drift, b_drift, c_drift),
...
mutable struct ParametersSIRK{DT, TT, D, M, S, ET <: NamedTuple} <: Parameters{DT, TT}
    ...
    function ParametersSIRK{DT, D, M}(
            equ::ET, tab::TableauSIRK{TT}, Δt::TT, ΔW::Vector{DT},
...
function Integrators.IntegratorCache{ST}(
        params::ParametersSIRK{DT, TT, D, M, S}; kwargs...) where {ST, DT, TT, D, M, S}
    IntegratorCacheSIRK{ST, D, M, S}(; kwargs...)
end

@inline Integrators.CacheType(ST,
    params::ParametersSIRK{DT, TT, D, M, S}) where {DT, TT, D, M, S} = IntegratorCacheSIRK{
    ST, D, M, S}
...
struct IntegratorSIRK{DT, TT, D, M, S,
    ...
    function IntegratorSIRK(params::ParametersSIRK{DT, TT, D, M, S},
            solver::ST, caches) where {DT, TT, D, M, S, ST}
        new{DT, TT, D, M, S, typeof(params), ST}(params, solver, caches)
    end

    function IntegratorSIRK{DT, D, M}(equations::NamedTuple, tableau::TableauSIRK{TT},
    ...
    function IntegratorSIRK(equation::SDE{DT, TT}, tableau::TableauSIRK{TT}, Δt::TT; kwargs...) where {
            DT, TT}
...
function Integrators.integrate_step!(
        int::IntegratorSIRK{DT, TT}, sol::AtomicSolutionSDE{DT, TT},
        cache::IntegratorCacheSIRK{DT} = int.caches[DT]) where {DT, TT}
    ...
    update_solution!(sol, cache.V, cache.B, tableau(int).qdrift.b,
        tableau(int).qdiff.b, int.params.Δt, int.params.ΔW, cache.Δy)
    update_solution!(sol, cache.V, cache.B, tableau(int).qdrift.b̂,
        tableau(int).qdiff.b̂, int.params.Δt, int.params.ΔW, cache.Δy)
end
