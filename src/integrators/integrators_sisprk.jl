...
struct TableauSISPRK{T} <: AbstractTableauIRK{T}
    ...
    function TableauSISPRK{T}(
            name, s, qdrift, qdiff, pdrift1, pdrift2, pdiff1, pdiff2) where {T}
...
function TableauSISPRK(name::Symbol, qorder_drift::Int, qa_drift::Matrix{T},
        qb_drift::Vector{T}, qc_drift::Vector{T},
...
mutable struct ParametersSISPRK{DT, TT, D, M, S, ET <: NamedTuple} <: Parameters{DT, TT}
    ...
    function ParametersSISPRK{DT, D, M}(
            equ::ET, tab::TableauSISPRK{TT}, Δt::TT, ΔW::Vector{DT},
...
function Integrators.IntegratorCache{ST}(
        params::ParametersSISPRK{DT, TT, D, M, S}; kwargs...) where {ST, DT, TT, D, M, S}
    IntegratorCacheSISPRK{ST, D, M, S}(; kwargs...)
end

@inline Integrators.CacheType(ST,
    params::ParametersSISPRK{DT, TT, D, M, S}) where {
    DT, TT, D, M, S} = IntegratorCacheSISPRK{ST, D, M, S}
...
struct IntegratorSISPRK{DT, TT, D, M, S,
    ...
    function IntegratorSISPRK(params::ParametersSISPRK{DT, TT, D, M, S},
            solver::ST, caches) where {DT, TT, D, M, S, ST}
        new{DT, TT, D, M, S, typeof(params), ST}(params, solver, caches)
    end

    function IntegratorSISPRK{DT, D, M}(equations::NamedTuple, tableau::TableauSISPRK{TT},
        ...
        # create params
        params = ParametersSISPRK{DT, D, M}(
    ...
    function IntegratorSISPRK(equation::SPSDE{DT, TT}, tableau::TableauSISPRK{TT},
...
function update_solution!(
        sol::AtomicSolutionPSDE{T}, tab::TableauSISPRK{T}, Δt::T, ΔW::Vector{T},
...
function Integrators.integrate_step!(
        int::IntegratorSISPRK{DT, TT}, sol::AtomicSolutionPSDE{DT, TT},
        cache::IntegratorCacheSISPRK{DT} = int.caches[DT]) where {DT, TT}
