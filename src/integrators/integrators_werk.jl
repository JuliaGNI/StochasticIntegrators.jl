...
struct TableauWERK{T} <: AbstractTableauERK{T}
    ...
    function TableauWERK{T}(
            name, qdrift0, qdrift1, qdrift2, qdiff0, qdiff1, qdiff2, qdiff3) where {T}
        @assert qdrift0.s == qdrift1.s == qdrift2.s == qdiff0.s == qdiff1.s == qdiff2.s ==
                qdiff3.s
        @assert qdrift0.c[1] == qdrift1.c[1] == qdrift2.c[1] == qdiff0.c[1] ==
                qdiff1.c[1] == qdiff2.c[1] == qdiff3.c[1] == 0.0
        ...
        @assert !(qdrift0.s==1.0 && qdrift0.a[1, 1] ≠ 0.0)
        @assert !(qdrift1.s==1.0 && qdrift1.a[1, 1] ≠ 0.0)
        @assert !(qdrift2.s==1.0 && qdrift2.a[1, 1] ≠ 0.0)
        @assert !(qdiff0.s==1.0 && qdiff0.a[1, 1] ≠ 0.0)
        @assert !(qdiff1.s==1.0 && qdiff1.a[1, 1] ≠ 0.0)
        @assert !(qdiff2.s==1.0 && qdiff2.a[1, 1] ≠ 0.0)
        @assert !(qdiff3.s==1.0 && qdiff3.a[1, 1] ≠ 0.0)
...
struct ParametersWERK{DT, TT, D, M, S, ET <: NamedTuple} <: Parameters{DT, TT}
    ...
    function ParametersWERK{DT, D, M}(
            equs::ET, tab::TableauWERK{TT}, Δt::TT) where {DT, TT, D, M, ET <: NamedTuple}
...
function Integrators.IntegratorCache{ST}(
        params::ParametersWERK{DT, TT, D, M, S}; kwargs...) where {ST, DT, TT, D, M, S}
    IntegratorCacheWERK{ST, D, M, S}(; kwargs...)
end

@inline Integrators.CacheType(ST,
    params::ParametersWERK{DT, TT, D, M, S}) where {DT, TT, D, M, S} = IntegratorCacheWERK{
    ST, D, M, S}
...
struct IntegratorWERK{DT, TT, D, M, S, ET} <: StochasticIntegratorRK{DT, TT, D, M, S}
    ...
    function IntegratorWERK(
            params::ParametersWERK{
                DT, TT, D, M, S, ET}, caches) where {DT, TT, D, M, S, ET}
        new{DT, TT, D, M, S, ET}(params, caches)
    end

    function IntegratorWERK{DT, D, M}(equations::ET, tableau::TableauWERK{TT},
            Δt::TT) where {DT, TT, D, M, ET <: NamedTuple}
    ...
    function IntegratorWERK(equation::SDE{DT, TT}, tableau::TableauWERK{TT}, Δt::TT; kwargs...) where {
            DT, TT}
...
function Integrators.integrate_step!(
        int::IntegratorWERK{DT, TT}, sol::AtomicSolutionSDE{DT, TT},
    ...
    for i in 2:tableau(int).s
        ...
        for k in eachindex(cache.Q1[1])
            ...
            for noise_idx in eachindex(cache.Q1)
                ...
                cache.Q1[noise_idx][k] = sol.q̄[k] + timestep(int) * ydrift +
                                         dot(cache.Δy, sol.ΔW)
            end
        end
            ...
            for noise_idx in eachindex(cache.Q2)
                ...
                for j in 1:(i - 1)
                    for l in eachindex(sol.ΔW, sol.ΔZ)
                        ...
                        if l<noise_idx
                            ydiff2 += tableau(int).qdiff2.a[i, j] * cache.B1[j][k, l] *
                                      sol.ΔW[noise_idx] * sol.ΔZ[l]
                        elseif l>noise_idx
                            ydiff2 += -tableau(int).qdiff2.a[i, j] * cache.B1[j][k, l] *
                                      sol.ΔW[l] * sol.ΔZ[noise_idx]
                        end
                    end
                end

                cache.Q2[noise_idx][k] = sol.q̄[k] + timestep(int) * ydrift +
                                         ydiff2/sqrt(timestep(int))
            end
        end
    ...
    update_solution!(sol, cache.V, cache.B1, cache.B2, tableau(int).qdrift0.b,
        tableau(int).qdiff0.b, tableau(int).qdiff3.b, timestep(int), sol.ΔW, cache.Δy)
