...
mutable struct ParametersWIRK{DT, TT, D, M, S, ET <: NamedTuple} <: Parameters{DT, TT}
    ...
    function ParametersWIRK{DT, D, M}(
            equ::ET, tab::TableauWIRK{TT}, Δt::TT, ΔW::Vector{DT},
            ΔZ::Vector{DT}) where {DT, TT, D, M, ET <: NamedTuple}
...
function Integrators.IntegratorCache{ST}(
        params::ParametersWIRK{DT, TT, D, M, S}; kwargs...) where {ST, DT, TT, D, M, S}
    IntegratorCacheWIRK{ST, D, M, S}(; kwargs...)
end

@inline Integrators.CacheType(ST,
    params::ParametersWIRK{DT, TT, D, M, S}) where {DT, TT, D, M, S} = IntegratorCacheWIRK{
    ST, D, M, S}
...
struct IntegratorWIRK{DT, TT, D, M, S,
    ...
    function IntegratorWIRK(params::ParametersWIRK{DT, TT, D, M, S},
            solver::ST, caches) where {DT, TT, D, M, S, ST}
        new{DT, TT, D, M, S, typeof(params), ST}(params, solver, caches)
    end

    function IntegratorWIRK{DT, D, M}(
            equations::NamedTuple, tableau::TableauWIRK{TT}, Δt::TT) where {DT, TT, D, M}
        # get number of stages
        S = tableau.s

        # create params
        params = ParametersWIRK{DT, D, M}(
            equations, tableau, Δt, zeros(DT, M), zeros(DT, M))
    ...
    function IntegratorWIRK(equation::SDE{DT, TT}, tableau::TableauWIRK{TT}, Δt::TT; kwargs...) where {
            DT, TT}
...
function compute_stages!(
        x::Vector{ST}, Q::AbstractMatrix{Vector{ST}}, V::Vector{Vector{ST}},
            ...
            for k in 1:D
                ...
                for j in 1:S
                    ...
                    for noise_idx in 1:M
                        if noise_idx==l
                            y1 += params.tab.qdiff1.a[i, j] * cache.B[j][k, noise_idx] *
                                  params.ΔW[noise_idx]
                            y2 += params.tab.qdiff1.â[i, j] * cache.B[j][k, noise_idx] *
                                  params.ΔW[noise_idx]
                        else
                            y1 += params.tab.qdiff3.a[i, j] * cache.B[j][k, noise_idx] *
                                  params.ΔW[noise_idx]
                            y2 += params.tab.qdiff3.â[i, j] * cache.B[j][k, noise_idx] *
                                  params.ΔW[noise_idx]
                        end
                    end
                end
                b[D * S + D * M * (i - 1) + D * (l - 1) + k] = - cache.Y[l, i][k] +
                                                               (y1 + y2)
            end
        end
    end
...
function Integrators.integrate_step!(
        int::IntegratorWIRK{DT, TT}, sol::AtomicSolutionSDE{DT, TT},
        cache::IntegratorCacheWIRK{DT} = int.caches[DT]) where {DT, TT}
    ...
    update_solution!(sol, cache.V, cache.B, tableau(int).qdrift0.b,
        tableau(int).qdiff0.b, timestep(int), int.params.ΔW, cache.Δy)
    update_solution!(sol, cache.V, cache.B, tableau(int).qdrift0.b̂,
        tableau(int).qdiff0.b̂, timestep(int), int.params.ΔW, cache.Δy)
end
