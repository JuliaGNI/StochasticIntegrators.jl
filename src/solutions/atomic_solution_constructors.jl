...
"Create AtomicSolution for SDE."
function Solutions.AtomicSolution(solution::SolutionSDE{
        AT, TT}) where {DT, TT, AT <: AbstractArray{DT}}
...
function Solutions.AtomicSolution(equation::AbstractEquationPSDE{DT, TT}) where {DT, TT}
    AtomicSolutionPSDE(equation.t₀, equation.q₀[begin], equation.p₀[begin],
        zeros(DT, equation.m), zeros(DT, equation.m))
end

"Create AtomicSolution for partitioned SDE."
function Solutions.AtomicSolution(solution::SolutionPSDE{
        AT, TT}) where {DT, TT, AT <: AbstractArray{DT}}
