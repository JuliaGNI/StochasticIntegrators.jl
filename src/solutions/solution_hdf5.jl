...
function Solutions.init_solution(h5::HDF5.File, solution::SolutionSDE{
        DT, TT, 2}) where {DT, TT}
    q = create_dataset(h5, "q", DT, ((solution.nd, solution.nt+1), (solution.nd, -1)),
        chunk = (solution.nd, 1))
    q[:, 1] = solution.q[:, 0]
end
...
function Solutions.init_solution(h5::HDF5.File, solution::SolutionSDE{
        DT, TT, 3}) where {DT, TT}
    q = create_dataset(h5, "q", DT,
        ((solution.nd, solution.nt+1, solution.ns), (solution.nd, -1, solution.ns)),
        chunk = (solution.nd, 1, 1))
    q[:, 1, :] = solution.q[:, 0, :]
end

function Solutions.init_solution(h5::HDF5.File, solution::SolutionPSDE{
        DT, TT, 2}) where {DT, TT}
    q = create_dataset(h5, "q", DT, ((solution.nd, solution.nt+1), (solution.nd, -1)),
        chunk = (solution.nd, 1))
    p = create_dataset(h5, "p", DT, ((solution.nd, solution.nt+1), (solution.nd, -1)),
        chunk = (solution.nd, 1))
    q[:, 1] = solution.q[:, 0]
    p[:, 1] = solution.p[:, 0]
end
...
function init_increments(h5::HDF5.File, solution::StochasticSolution{
        DT, TT, NQ, 2}) where {DT, TT, NQ}
    dW = create_dataset(h5, "ΔW", DT, ((solution.nm, solution.ntime), (solution.nm, -1)),
        chunk = (solution.nm, 1))
    dZ = create_dataset(h5, "ΔZ", DT, ((solution.nm, solution.ntime), (solution.nm, -1)),
        chunk = (solution.nm, 1))
end

function init_increments(h5::HDF5.File, solution::StochasticSolution{
        DT, TT, NQ, 3}) where {DT, TT, NQ}
    dW = create_dataset(h5, "ΔW", DT,
        ((solution.nm, solution.ntime, solution.ns), (solution.nm, -1, solution.ns)),
        chunk = (solution.nm, 1, 1))
    dZ = create_dataset(h5, "ΔZ", DT,
        ((solution.nm, solution.ntime, solution.ns), (solution.nm, -1, solution.ns)),
        chunk = (solution.nm, 1, 1))
end
...
function Solutions.save_solution(h5::HDF5.File, solution::SolutionSDE{DT, TT, 1},
        j1, j2, n1, n2) where {DT <: Number, TT}
    if size(h5["q"])[end] < j2
...
function Solutions.save_solution(h5::HDF5.File, solution::SolutionSDE{AT, TT, 1},
        j1, j2, n1, n2) where {DT, AT <: Array{DT}, TT}
...
function Solutions.save_solution(h5::HDF5.File, solution::SolutionSDE{DT, TT, 2},
        j1, j2, n1, n2) where {DT <: Number, TT}
    if size(h5["q"], 1) < j2
...
function Solutions.save_solution(h5::HDF5.File, solution::SolutionSDE{AT, TT, 2},
        j1, j2, n1, n2) where {DT, AT <: Array{DT}, TT}
    elaxes = axes(solution.q[begin, begin])
    if size(h5["q"])[end - 1] < j2
        HDF5.set_extent_dims(h5["q"], (
            size(h5["q"])[begin:(end - 2)]..., j2, size(h5["q"])[end]))
    end
...
function Solutions.save_solution(h5::HDF5.File, solution::SolutionPSDE{DT, TT, 1},
        j1, j2, n1, n2) where {DT <: Number, TT}
    if size(h5["q"])[end] < j2
        HDF5.set_extent_dims(h5["q"], (j2,))
    end
...
function Solutions.save_solution(h5::HDF5.File, solution::SolutionPSDE{AT, TT, 1},
        j1, j2, n1, n2) where {DT, AT <: Array{DT}, TT}
    elaxes = axes(solution.q[begin])
    if size(h5["q"])[end] < j2
        HDF5.set_extent_dims(h5["q"], (size(h5["q"])[1:(end - 1)]..., j2))
...
function Solutions.save_solution(h5::HDF5.File, solution::SolutionPSDE{DT, TT, 2},
        j1, j2, n1, n2) where {DT <: Number, TT}
    if size(h5["p"], 1) < j2
...
function Solutions.save_solution(h5::HDF5.File, solution::SolutionPSDE{AT, TT, 2},
        j1, j2, n1, n2) where {DT, AT <: Array{DT}, TT}
    elaxes = axes(solution.q[begin, begin])
    if size(h5["q"])[end - 1] < j2
        HDF5.set_extent_dims(h5["q"], (
    ...
    if size(h5["p"])[end - 1] < j2
        HDF5.set_extent_dims(h5["p"], (
            size(h5["p"])[begin:(end - 2)]..., j2, size(h5["p"])[end]))
    end
...
function save_increments(h5::HDF5.File, solution::StochasticSolution{DT, TT, NQ, 2},
        j1, j2, n1, n2) where {DT, TT, NQ}
    if size(h5["ΔW"], 2) < j2
...
function save_increments(h5::HDF5.File, solution::StochasticSolution{DT, TT, NQ, 3},
        j1, j2, n1, n2) where {DT, TT, NQ}
    if size(h5["ΔW"], 2) < j2
...
"""
function write_to_hdf5(solution::StochasticSolution, h5::HDF5.File = hdf5(solution),
        soffset = offset(solution), woffset = ioffset(solution))
