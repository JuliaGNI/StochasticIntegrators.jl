...
"""
mutable struct SolutionPSDE{dType,tType,wType,NQ,NW,CONV} <: StochasticSolution{dType,tType,wType,NQ,NW}
    nd::Int
    nm::Int
    nt::Int
    ns::Int
    ...
    p::DataSeries{dType,NQ}
    W::WienerProcess{CONV}#WienerProcess{wType,tType,NW,CONV}
    K::Int
    ntime::Int
    nsave::Int
    nwrite::Int
    ...
    function SolutionPSDE(t::TimeSeries{TT}, q::DataSeries{DT,NQ}, p::DataSeries{DT,NQ}, W::WienerProcess{WT,TT,NW,CONV}; K::Int=0) where {DT,TT,WT,NQ,NW,CONV}
        # extract parameters
        nd = q.nd
        ns = q.ni
        nt = q.nt
    ...
    function SolutionPSDE(::Type{dType}, nd::Int, nm::Int, nt::Int, ns::Int, ni::Int, Δt::tType,
        ...
        p = DataSeries(dType, nd, nt, max(ns, ni))
        NQ = ns == ni == 1 ? 1 : 2
...
function SolutionPSDE(equation::Union{PSDE{DT,TT},SPSDE{DT,TT}}, Δt::TT, ntime::Int; nsave::Int=DEFAULT_NSAVE, nwrite::Int=DEFAULT_NWRITE, K::Int=0, conv=DEFAULT_SCONV, filename=nothing) where {DT,TT}
    ...
    s = SolutionPSDE(DT, nd, nm, nt, ns, ni, Δt, W, K, ntime, nsave, nw, periodicity(equation))
...
function SolutionPSDE(t::TimeSeries{TT}, q::DataSeries{DT,NQ}, p::DataSeries{DT,NQ}; K::Int=0, conv=DEFAULT_SCONV) where {DT,TT,NQ}
    # extract parameters
    nd = q.nd
    ns = q.ni
    nt = t.n
...
function Solutions.get_initial_conditions!(sol::SolutionPSDE{AT,TT}, asol::SolutionStepPSDE{DT,TT,AT}, k, n=1) where {DT,TT,AT<:AbstractArray{DT}}
...
function Solutions.set_solution!(sol::SolutionPSDE{AT,TT}, asol::SolutionStepPSDE{DT,TT,AT}, n, k=1) where {DT,TT,AT<:AbstractArray{DT}}
...
# copy increments of the Brownian Process for multidimensional Brownian motion, 1 sample path
function get_increments!(sol::SolutionPSDE{AT,TT,WT,NQ,2}, asol::SolutionStepPSDE{DT,TT,AT}, n, k=1) where {DT,TT,AT,WT,NQ}
    @assert k == 1
    for l = 1:sol.nm
...
# copy increments of the Brownian Process for multidimensional Brownian motion, r-th sample path
function get_increments!(sol::SolutionPSDE{AT,TT,WT,NQ,3}, asol::SolutionStepPSDE{DT,TT,AT}, n, k) where {DT,TT,AT,WT,NQ}
    for l = 1:sol.nm
