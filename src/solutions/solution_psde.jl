...
"""
abstract type AbstractSolutionPSDE{dType, tType, wType, NQ, NW, CONV} <:
              StochasticSolution{dType, tType, wType, NQ, NW} end

# Create SolutionPSDEs with serial and parallel data structures.
for (TSolution, TDataSeries, Tdocstring) in ((:SolutionPSDE, :DataSeries,
    "Serial Solution of a partitioned stochastic differential equation."),
#  (:PSolutionPSDE, :PDataSeries, "Parallel Solution of a partitioned stochastic differential equation.")
    ...
    @eval begin
        ...
        mutable struct $TSolution{dType, tType, wType, NQ, NW, CONV} <:
                       AbstractSolutionPSDE{dType, tType, wType, NQ, NW, CONV}
            nd::Int
            nm::Int
            nt::Int
            ns::Int
            ...
            function $TSolution(t::TimeSeries{TT}, q::$TDataSeries{DT, NQ},
                    p::$TDataSeries{DT, NQ}, W::WienerProcess{WT, TT, NW, CONV};
                ...
                new{DT, TT, WT, NQ, NW, CONV}(nd, nm, nt, ns, t, q, p, W, K, ntime,
                    nsave, nwrite, zeros(Int, ns), 0, 0)
            end
            ...
            function $TSolution(
                ...
                new{Vector{dType}, tType, wType, NQ, NW, CONV}(
                    nd, nm, nt, max(ns, ni), t, q, p, W, K, ntime, nsave,
                    nwrite, zeros(Int, max(ns, ni)), 0, 0, periodicity)
            end
        end

        function $TSolution(
                equation::Union{PSDE{DT, TT}, SPSDE{DT, TT}}, Δt::TT, ntime::Int;
        ...
        function $TSolution(equation::Union{PSDE{DT, TT}, SPSDE{DT, TT}}, Δt::TT,
                dW::Array{DT, NW}, dZ::Array{DT, NW}, ntime::Int;
                nsave::Int = DEFAULT_NSAVE, nwrite::Int = DEFAULT_NWRITE, K::Int = 0,
                conv = DEFAULT_SCONV, filename = nothing) where {DT, TT, NW}
            nd = equation.d
            nm = equation.m
            ns = equation.ns
            ni = nsamples(equation)
        ...
        function $TSolution(
                t::TimeSeries{TT}, q::$TDataSeries{DT, NQ}, p::$TDataSeries{DT, NQ};
                K::Int = 0, conv = DEFAULT_SCONV) where {DT, TT, NQ}
            # extract parameters
            nd = q.nd
            ns = q.ni
            nt = t.n
...
function Base.:(==)(sol1::SolutionPSDE{DT1, TT1, NQ1, NW1, C1},
        sol2::SolutionPSDE{DT2, TT2, NQ2, NW2, C2}) where {
...
@inline Solutions.lastentry(sol::SolutionPSDE) = sol.ni == 1 ? sol.counter[1] - 1 :
                                                 sol.counter .- 1
...
function Solutions.set_initial_conditions!(
        sol::SolutionPSDE{AT, TT, WT, 1}, t₀::TT, q₀::AT, p₀::AT) where {AT, TT, WT}
    set_data!(sol.q, q₀, 0)
    set_data!(sol.p, p₀, 0)
...
function Solutions.set_initial_conditions!(
        sol::SolutionPSDE{AT, TT, WT, 2}, t₀::TT, q₀::AT, p₀::AT) where {AT, TT, WT}
    for k in 1:sol.ns
...
function Solutions.set_initial_conditions!(
        sol::SolutionPSDE{AT, TT, WT}, t₀::TT, q₀::AbstractVector{AT},
        p₀::AbstractVector{AT}) where {AT, TT, WT}
...
function Solutions.get_initial_conditions!(
        sol::SolutionPSDE{AT, TT}, asol::AtomicSolutionPSDE{DT, TT, AT},
...
function Solutions.get_initial_conditions!(
        sol::SolutionPSDE{AT}, q::AT, p::AT, k, n = 1) where {AT}
...
function Solutions.get_solution(sol::SolutionPSDE{AT, TT, WT, 1}, n, k = 1) where {
        AT, TT, WT}
    @assert k == 1
    (sol.t[n], sol.q[n], sol.p[n])
end

function Solutions.get_solution(sol::SolutionPSDE{AT, TT, WT, 2}, n, k = 1) where {
        AT, TT, WT}
    (sol.t[n], sol.q[n, k], sol.p[n, k])
end

# copy increments of the Brownian Process for multidimensional Brownian motion, 1 sample path
function get_increment(sol::SolutionPSDE{AT, TT, WT, NQ, 2}, n, k = 1) where {
        AT, TT, WT, NQ}
    @assert k==1
...
# copy increments of the Brownian Process for multidimensional Brownian motion, 1 sample path
function get_increments!(sol::SolutionPSDE{AT, TT, WT, NQ, 2},
        asol::AtomicSolutionPSDE{DT, TT, AT}, n, k = 1) where {DT, TT, AT, WT, NQ}
    @assert k==1
    for l in 1:sol.nm
...
# copy increments of the Brownian Process for multidimensional Brownian motion, r-th sample path
function get_increments!(sol::SolutionPSDE{AT, TT, WT, NQ, 3},
        asol::AtomicSolutionPSDE{DT, TT, AT}, n, k) where {DT, TT, AT, WT, NQ}
    for l in 1:sol.nm
