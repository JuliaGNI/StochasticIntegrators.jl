...
struct WienerProcess{dType, tType, N, CONV} <: SemiMartingale{dType, tType, N}
    ...
    function WienerProcess{dType, tType, N, CONV}(nd, nt, ns, Δt,
            ...
            ΔZ = (ns == 1 ? zeros(dType, nd, nt) : zeros(dType, nd, nt, ns))) where {
            dType, tType, N, CONV}
...
function WienerProcess(dType, nd, nt, ns, Δt::tType, conv = DEFAULT_SCONV;
        rng = GLOBAL_RNG) where {tType <: Number}
    wp = WienerProcess{dType, tType, ns==1 ? 2 : 3, conv}(nd, nt, ns, Δt)
    generate_wienerprocess!(wp, rng)
    return wp
end

function WienerProcess(Δt::tType, dW::Array{dType, 1}, dZ::Array{dType, 1},
        conv = DEFAULT_SCONV) where {dType, tType}
    ...
    return WienerProcess{dType, tType, 2, conv}(
        nd, nt, ns, Δt, reshape(dW, (1, nt)), reshape(dZ, (1, nt)))
end

function WienerProcess(Δt::tType, dW::Array{dType, N}, dZ::Array{dType, N},
        conv = DEFAULT_SCONV) where {dType, tType, N}
...
function Base.:(==)(wp1::WienerProcess{DT1, TT1, N1, C1},
        wp2::WienerProcess{DT2, TT2, N2, C2}) where {DT1, TT1, N1, C1, DT2, TT2, N2, C2}
    (
        DT1 == DT2
        && TT1 == TT2
        && N1 == N2
...
function set_chi_and_eta!(
        W::WienerProcess{dType, tType, N, :strong}, chi::AbstractArray{dType},
        eta::AbstractArray{dType}) where {dType, tType, N}
    @. W.ΔW = chi * √W.Δt
...
function set_chi_and_eta!(
        W::WienerProcess{dType, tType, N, :weak}, chi::AbstractArray{dType},
        eta::AbstractArray{dType}) where {dType, tType, N}
    W.ΔW .= 0
    W.ΔZ .= sqrt(W.Δt)
    ...
    # P(\\hat I=0) = 2/3
    indx = findall(x->x<1.0 / 6.0, chi)
    W.ΔW[indx] .= -sqrt(3*W.Δt)
    indx = findall(x->x>5.0 / 6.0, chi)
    W.ΔW[indx] .= sqrt(3*W.Δt)
...
function generate_wienerprocess!(W::WienerProcess{DT, TT, 2, :strong}, rng = GLOBAL_RNG) where {
        DT, TT}
...
# Generates a new series of increments for the strong Wiener process W
function generate_wienerprocess!(W::WienerProcess{DT, TT, 3, :strong}, rng = GLOBAL_RNG) where {
        DT, TT}
...
function generate_wienerprocess!(W::WienerProcess{DT, TT, 2, :weak}, rng = GLOBAL_RNG) where {
        DT, TT}
...
# Generates a new series of increments for the weak Wiener process W
function generate_wienerprocess!(W::WienerProcess{DT, TT, 3, :weak}, rng = GLOBAL_RNG) where {
        DT, TT}
